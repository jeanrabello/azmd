...
**About the popover on Windows.** Anchoring it *under* the icon is right on macOS, where the menu bar is at the top — and wrong on Windows, where the taskbar sits at the bottom by default: the window landed past the bottom edge and only a sliver of it was visible. `#positionNearTray` now tries below the icon and falls back to above when it doesn't fit, then clamps on both axes, which also covers a side taskbar and an icon inside the hidden-icons flyout. Two smaller things follow from the same platform difference: clicking the tray icon while the popover is open fires blur *before* click on Windows, so the blur hid the window and the click reopened it — a 250 ms guard makes the icon close what it opened; and notifications need an AppUserModelID (`app.setAppUserModelId`) or the Action Center drops them silently, which is why the test notification never showed in dev.

**About PATH and the Azure CLI (Windows).** Nothing to recover here — the PATH comes from the registry and is the same for GUI and terminal. The repair still runs, for two reasons: the CLI installer only appends its `wbin` directory to the *user* PATH, which sessions started before the install don't see; and `az` on Windows is `az.cmd`, so looking for a file literally named `az` would never find it — `findExecutable` resolves the name through `PATHEXT`, the same rule `cmd` uses. The login-shell fallback is skipped.
